...
O vídeo demonstrativo pode seguir a seguinte ordem:

1. Apresentar rapidamente o objetivo do sistema.
2. Mostrar a estrutura do projeto e a separação de camadas.
3. Mostrar o `Dockerfile` e o `docker-compose.yml`.
4. Executar ou mostrar a aplicação rodando com Docker Compose.
5. Acessar o Swagger.
6. Mostrar o endpoint público de acompanhamento da OS.
7. Demonstrar uma alteração de status da OS.
8. Mostrar no log a notificação simulada de e-mail.
9. Mostrar os manifestos Kubernetes no diretório `/k8s`.
10. Executar ou mostrar os pods rodando com:

```bash
kubectl get all -n mecanica
```

11. Mostrar o acesso via port-forward:

```bash
kubectl port-forward service/mecanica-api 8080:8080 -n mecanica
```

12. Mostrar os arquivos Terraform no diretório `/infra`.
13. Executar ou mostrar:
...
14. Mostrar os recursos provisionados pelo Terraform:

```bash
kubectl get all -n mecanica-tf
```

15. Mostrar `kubectl top pods -n mecanica-tf` e o HPA com métricas numéricas.
16. Gerar carga, mostrar o Deployment aumentando de uma para mais réplicas e remover o pod `carga-hpa`.
17. Mostrar o GitHub Actions criando o cluster, aplicando os YAMLs e validando o health check.
18. Encerrar explicando que a entrega contempla backend, Docker, Kubernetes, Terraform, CI e notificação simulada.

## Checklist da entrega

- [x] Refatoração da aplicação da Fase 1.
- [x] Separação em camadas.
- [x] Testes automatizados.
- [x] Relatório de cobertura com JaCoCo.
- [x] Dockerfile.
- [x] Docker Compose.
- [x] PostgreSQL.
- [x] Swagger/OpenAPI.
- [x] Autenticação JWT.
- [x] Rota pública para acompanhamento da OS.
- [x] Abertura de OS com cliente, veículo, serviços e peças.
- [x] Gestão de estoque.
- [x] Aprovação e recusa de orçamento.
- [x] Callbacks externos públicos para aprovação e recusa.
- [x] Controle de status da OS.
- [x] Listagem priorizada por status, mais antigas primeiro, sem finalizadas ou entregues.
- [x] Notificação simulada ao alterar status.
- [x] Kubernetes com Deployment, Service, ConfigMap, Secret, PVC e HPA por CPU/memória.
- [x] Terraform para criação de cluster Kind e provisionamento dos recursos Kubernetes.
- [x] GitHub Actions para build, testes, Docker, Terraform e deploy Kubernetes.
- [x] README com instruções de execução e arquitetura.
- [ ] Inserir o link definitivo do vídeo no README e no PDF enviado ao portal.
